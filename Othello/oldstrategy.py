...
def placeChar(indx, board, sym):
    ...
    return newPzl
def negaMax(brd, token, indx, level, player):
    poss = genPossible(brd, token)
    poss2 = genPossible(brd, switch[token])
    score = brd.count(player) - brd.count(switch[player])
    ...
    nmList = [negaMax(placeChar(new_indx, brd, token), switch[token], [new_indx] + indx, level-1, player) for new_indx in poss]
...
def midHeuristic(board, token):
    n = 0
    poss, poss2 = genPossible(board, switch[token]), genPossible(board, token)
    for c in corners:
        if board[c] == token: n += 10000
        elif board[c] == token: n -= 10000
        elif c in poss: n -= 50
        elif c in poss2: n += 50
    for num in range(64):
        if board[num] == token:
...
def heuristicMove(board, token, indx):
    ...
    print(indx, n)
    return n
def midGame(brd, token, lower, upper, level):
    poss = genPossible(brd, token)
    poss2 = genPossible(brd, switch[token])
    score = midHeuristic(brd, token)
    if not poss or not level:
        if not poss2 or not level: return [score]
        result = midGame(brd, switch[token], -upper, -lower, level-1)
        return [-result[0]] + result[1:] + [-1]
    best = [lower-1]
    for move in poss:
        nm = midGame(placeChar(move, brd, token), switch[token], -upper, -lower, level-1)
        score = -nm[0]
        if score > upper: return [score]
        if score < lower: continue
        best = [score] + nm[1:] + [move]
...
class Strategy():
    def best_strategy(self, board, player, best_move, running):
        if running.value:
            ...
            if (pzl, player) in moveSel:
                print('Openbook Move')
                move = moveSel[(pzl, player)]
            elif (pzl90, player) in moveSel:
                print('Openbook Move')
                move = calculatePosition(moveSel[(pzl90, player)], 90)
            elif (pzl180, player) in moveSel:
                print('Openbook Move')
                move = calculatePosition(moveSel[(pzl180, player)], 180)
            elif (pzl270, player) in moveSel:
                print('Openbook Move')
                move = calculatePosition(moveSel[(pzl270, player)], 270)
            elif period > 11:
                print('Heuristic Move')
                move = max([(heuristicMove(placeChar(x, pzl, player), player, x), x) for x in genPossible(pzl, player)])[1]
            else:
                move = alphaBeta(pzl, player, -64, 64)[-1]
                print('AlphaBeta Move')
            best_move.value = newIndexing[move]
            printFormat(board, 10)
            print("('", pzl, "', '", player, "')", " : ", move, sep="")
            print('Moving at', move)