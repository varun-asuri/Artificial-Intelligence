...
def readDimensions(extend):
    letters = []
    for a in ALPHA[:len(USED)]:
        letters.append( (extend.find(a), a) )
    letters.sort()
    for i, a in letters:
        height = 0
        width = 0
        for line in CURRENT[-1][i//CONTAINER[1]:]:
            temp = line.count(a)
            if temp:
                width = temp
                height += 1
