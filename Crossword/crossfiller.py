...
board, high = '-' * dimensions[2], dimensions[2]
buffers = int(sys.argv[2])
best = dimensions[2]-buffers
dictionary = [set() for a in range(dimensions[4]-2)]
preprocess = [set() for b in range(dimensions[4]*26)]
...
for raw in file:
    ...
    for d, o in enumerate(nL):
        if o in alpha: preprocess[d*26+alphadict[o]].add(nL)
words = sys.argv[4:]
for raw in words:
...
for a in range(dimensions[0]): print(board[a*dimensions[1]:(a+1)*dimensions[1]])
print('\nconstTime: ', startTime[1]-startTime[0], sep='')
print('solveTime: ', time.time()-startTime[1], sep='')
print('totalTime: ', time.time()-startTime[0], sep='')